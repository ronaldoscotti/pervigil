...
## Self-review fix (stage 7)

Self-review caught two "degrade, don't fake" violations, now fixed and tested:

- `SystemRunner` ignored the child's exit status, so a **failed** `tmux select-pane` (stale
  pane id) or an unreachable app still reported success. It now treats a non-zero exit as an
  error, so the focuser can react.
- On a raise failure the backend handed back a resume string it had **not** copied, while the UI
  toast claimed "copied instead." Now a failed precise tier **degrades to the clipboard floor** —
  it actually copies — and only a total failure (even the floor) surfaces an error, with honest
  toast wording. Two new tests cover both paths (`focus_with`).
